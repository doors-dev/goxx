...
The `github.com/doors-dev/goxx/hx` package wires GoX fragments to HTMX
attributes. A fragment handler is a top-level package function that receives an
`hx.Responser` and the request, then returns the fragment to render.

```go
import "github.com/doors-dev/goxx/hx"

elem Fragment(w hx.Responser, r *http.Request) {
    ~{
        w.Header().Set("X-Fragment", "ok")
...
/*
alternative style:
func Fragment(w hx.Responser, r *http.Request) gox.Elem {
    w.Header().Set("X-Fragment", "ok")
    w.SetStatus(http.StatusCreated)
    return <span>ok</span>
}
...
elem Page() {
    ...
    <div id="result"></div>
    <button (hx.Post(Fragment)) hx-target="#result">Post!</button>
}
```

Mount `hx.Handler` under `hx.Prefix()`. Router syntax depends on the router:

```go
// Standard library ServeMux.
mux.HandleFunc(hx.Prefix(), hx.Handler)

// Gorilla mux.
r.HandleFunc(hx.Prefix()+"{id}", hx.Handler)
```

All common HTMX verbs are available as attribute modifiers:

```go
elem Controls() {
    <button (hx.Get(Load)) hx-target="#result">Load</button>
    <button (hx.Post(Create)) hx-target="#result">Create</button>
    <button (hx.Put(Replace)) hx-target="#result">Replace</button>
    <button (hx.Patch(Update)) hx-target="#result">Update</button>
    <button (hx.Delete(Remove)) hx-target="#result">Remove</button>
}
```

> Handlers must be named top-level package functions. Closures, method values, and
other dynamic functions are rejected. This keeps the endpoint registry bounded:
rendering the same page repeatedly reuses the same fixed handler IDs.

### Preregistering Handlers

Rendering a page registers the handlers referenced by its `hx.*` modifiers. This
is convenient for single-process apps, but it can be risky behind a load
balancer: the browser may request a fragment from an app instance that has not
rendered that page yet. In load-balanced deployments, preregister each fragment
handler during startup. The usual pattern is `hx.MustRegister` in package
`init`:

```go
func init() {
    hx.MustRegister(Load)
    hx.MustRegister(Create)
    hx.MustRegister(Replace)
    hx.MustRegister(Update)
    hx.MustRegister(Remove)
}
```

You can still use `hx.Get`, `hx.Post`, and the other modifiers in templates;
they reuse the same fixed handler IDs and choose the HTMX request method.

### Configuration

The default handler prefix is `"/~/"`. If you want a different prefix,
configure it before rendering pages or serving fragments:
...
Use `SetOptions` to pass `goxx.Render` options used by fragment responses:

```go
hx.SetOptions(goxx.WithWorkers(16))
```
